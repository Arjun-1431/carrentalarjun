...
export default function BookingPage() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/getBookDetails')
      .then(response => {
        setBookcars(response.data);
      })
      .catch(error => {
        console.log(error);
      });
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://localhost:5000/deleteBooking/${id}`)
      .then(response => {
        setBookcars(prevBookcars => prevBookcars.filter(car => car._id !== id));
        Swal.fire({
          icon: "success",
          title: "Success...",
                  ...
                  >
                    <MaterialTable
                      ...
                      columns={[
                        ...
                        { title: 'Issue Date', field: 'issue', type: 'date' },
                        { title: 'Return Date', field: 'returnDate', type: 'date' },
                      ]}
                      data={bookcars}
                      actions={[
                        {
