...
export default function AllFeedback() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5001/getallFeedbacks')
      .then(response => {
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`http://localhost:5001/deleteFeedaback/${id}`)
      .then(response => {
        Swal.fire({
          icon: "success",
          title: "Success...",
        ...
        setAllFeedback(prevUsers => prevUsers.filter(user => user._id !== id));
      })
      .catch(err => {
